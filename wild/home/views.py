...
def wildopedia(request): 
    ...
    # Initialize cmt as an empty list, so it is always defined
    cmt_list = []
    
    if pst:
        # Loop through posts and collect comments for each post
        for post in pst:
            cmt = Reaction.objects.filter(pst=post.id)
            cmt_list.append(cmt)
    
    return render(request, "wildopedia.html", {"pkey": pst, "cmt": cmt_list})
